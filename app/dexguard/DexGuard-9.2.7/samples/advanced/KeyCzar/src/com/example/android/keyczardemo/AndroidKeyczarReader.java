...
public class AndroidKeyczarReader implements KeyczarReader {
    ...
    private String getFileContentAsString(String filename) throws KeyczarException {
        try {
            ...
            final String assetName = getFullFilename(filename);

            // DexGuard can encrypt hard-coded asset files for us,
            // so we explicitly list the ones that we want to encrypt.
            final InputStream is =
                assetName.equals("keys/meta") ? mAssetManager.open("keys/meta") :
                assetName.equals("keys/1")    ? mAssetManager.open("keys/1")    :
